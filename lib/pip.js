...
function installRequirementsFile(
  ...
  if (options.usePipenv && fse.existsSync(path.join(servicePath, 'Pipfile'))) {
    generateRequirementsFile(path.join(servicePath, '.serverless/requirements.txt'), targetFile, options);
...
function installRequirementsIfNeeded(servicePath, modulePath, options, serverless) {
  ...
  if (modulePath && modulePath != ".") {
    ...
    const destinationFile = path.join(servicePath, '.serverless', modulePath, 'requirements.txt');
  ...
  if (fse.existsSync(workingReqsFolder)) {
    if (fse.existsSync(path.join(workingReqsFolder, '.completed_requirements')) && workingReqsFolder.endsWith('_slspyc')) {
    ...
    values(this.serverless.service.functions)
      ...
      .map(f => {
        ...
        if (!doneModules.includes(f.module)) {
          ...
          let modulePath = path.join(this.servicePath, '.serverless', `${f.module}`, 'requirements');
          if (reqsInstalledAt && !fse.existsSync(modulePath) && reqsInstalledAt != modulePath) {
            if (this.options.staticCache) {
